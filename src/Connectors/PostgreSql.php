...
class PostgreSql extends \App\Connector
{
    /**
     * @var \PDO
     */
    ...
    {   
        ...
        if (!$this->invalidate_op) {
            ...
            $this->startTime("connection_time_pgsql");
            try {
                $pdo = new \PDO("pgsql:host=$host;port=$port", $user, $pass, array(
                    PDO::ATTR_TIMEOUT => 5,
                    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
                ));
                $this->connector = $pdo;
                $this->finishTime("connection_time_pgsql");
                return true;
            } catch (\PDOException $e) {
                $this->invalidate_op = true;
                $this->log("Connection - PostgreSQL", "Error", $e->getCode(), $e->getMessage());
                return false;
            }
        }
        return false;
